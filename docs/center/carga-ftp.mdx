...
###  Parâmetros de Conexão
 ...
 Acesse as configurações de FTP na plataforma:
...
[Acesse a página de configurações](https://inveniocenter.robbu.global/painel/configuracoes)
 ...
 Preencha os seguintes campos:
   ...
   - **Porta**: `FTP → porta padrão 21` ou `SFTP → porta padrão 22`
   - **Modo de criptografia**: A escolha depende das exigências do servidor utilizado. O tipo mais comum é **Use Explícito**.

>⚠️ **Atenção**:
>
> Ao configurar, não inclua `https://` no início da URL. Informe apenas o endereço do servidor (exemplo: ftp.seudominio.com).
>
...
> ⚠️ **Atenção**  
> O servidor disponibilizado pela **Invenio** é do tipo **SFTP**.  
> Portanto, inclua no início da URL o prefixo `sftp://`.  
>
> **Exemplo:**  
> `sftp://sarobbuinveniosftp.xyz.core.windows.net`  
>
...
| `_x999`                        | Descarta clientes com qualquer contato receptivo nas últimas `999` horas. <br /><span style={{ fontSize: '0.8em', opacity: 0.85, display: 'block' }}>⚠️ <strong>Atenção:</strong> A nomenclatura deste parâmetro deve ser definida antes da inclusão do parâmetro <code>_nt</code>, que corresponde ao nome do template.</span> |
...
| `_whatsappprospect`            | Campanha via módulo WhatsApp Prospect. Obrigatório informar o template. <br /><span style={{ fontSize: '0.8em', opacity: 0.85, display: 'block' }}>❗<strong>Observação:</strong> Utilize _whatsappprospect apenas quando houver números prospects, caso contrário, ocorrerá falha. Nesta situação, utilize o parâmetro <code>_whatsappexclusivo</code> junto com a especificação do número pelo parâmetro <code>_nw</code> .</span> |
...
- [Agendamento de relatório por FTP](https://docs.robbu.global/docs/center/agendamento-relatorio-ftp)  
