...
| `ContactWhatsAppBsuid` | Identificador do contato gerado pela Meta | `String` |
| `ContactWhatsAppBusinessManagementId` | Portfólio de negócios a que o identificador pertence | `String` |
...
<Note>
**Identificação de contatos do WhatsApp sem telefone.** O WhatsApp permite que a pessoa converse com a empresa usando username, sem expor o número. Quando isso acontece, o telefone não é informado e o contato passa a ser identificado pelo `bsuid`, um código gerado pela Meta que é exclusivo por portfólio de negócios. O `businessManagementId` indica a qual portfólio aquele `bsuid` pertence.

Se a sua integração reconhece a pessoa pelo número de telefone, passe a considerar também esses campos. O mesmo contato tem `bsuid` diferente em portfólios diferentes.
</Note>
  ...
  <Accordion title="Modelo JSON — Mensagem de Entrada (Incoming)">
    ...
    "message": {
      ...
      "contact": {
        ...
        "whatsapp": {
          "bsuid": "BR.3087873511408224",
          "businessManagementId": "104839572610394"
        }
      }
    }
    ```
...
| `ContactStatusWhatsAppBsuid` | Identificador do contato gerado pela Meta | `String` |
| `ContactStatusWhatsAppBusinessManagementId` | Portfólio de negócios a que o identificador pertence | `String` |
