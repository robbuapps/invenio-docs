...
| **NUMERO_CONTRATO** | ❌                                     | Código do contrato do cliente.                                                               |
| **NUMERO_TELEFONE** | ✅ se e-mail não estiver preenchido    | Número de telefone com DDD do cliente.                                                       |
| **EMAIL**           | ✅ se telefone não estiver preenchido  | E-mail do cliente.                                                                           |

Exemplo:
```csv
CPF_CNPJ;NUMERO_CONTRATO;NUMERO_TELEFONE;EMAIL
...
| Campo               | Obrigatório                                    | Descrição                                                                                    |
| ------------------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **TIPO_BLOQUEIO**   | ✅ se DATA_EXPIRACAO não for preenchido        | 1] Bloqueio permanente / 2] Bloqueio temporário / 3] Desbloqueio                             |
| **DATA_SOLICITACAO**| ❌                                             | Data de solicitação do bloqueio.                                                             |
| **DATA_EXPIRACAO**  | ✅ se TIPO_BLOQUEIO não for preenchido         | Data de expiração do bloqueio.                                                               |
| **NUMERO_CONTRATO** | ❌                                             | Nümero do contrato do cliente.                                                               |
| **EMAIL**           | ✅ se NUMERO_TELEFONE não estiver preenchido   | E-mail do cliente.                                                                           |
| **NUMERO_TELEFONE** | ✅ se EMAIL não estiver preenchido             | Número de telefone com DDD do cliente.                                                       |
| **MOTIVO**          | ❌                                             | Motivo do bloqueio.                                                                          |

Exemplo:
```csv
TIPO_BLOQUEIO;DATA_SOLICITACAO;DATA_EXPIRACAO;NUMERO_CONTRATO;EMAIL;NUMERO_TELEFONE;MOTIVO
2;05/03/2026;;12313213213;cliente@dominio.com;;Motivo 1
2;05/03/2026;;45665465464;;+5511930457438;
```
