...
### Destinatário: telefone (`to`) ou BSUID (`recipient`) <a href="#recipient" id="recipient"></a>

Todas as rotas de envio de mensagem aceitam **duas formas** de identificar o destinatário. Use **uma OU outra**:

| Campo       | Tipo   | Obrigatoriedade                          | Descrição                                                                                                                                                  |
| ----------- | ------ | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `to`        | string | Obrigatório se `recipient` for omitido   | Telefone do destinatário no formato internacional (ex.: `+5511999999999`).                                                                                |
| `recipient` | string | Opcional — envie no lugar de `to`          | **BSUID** (*Business-Scoped User ID*) ou **parent BSUID** do destinatário (ex.: `BR.1234567890`). Veja [BSUID e identificadores de usuário](/docs/positus/integracao/bsuid). |

<Info>
O **BSUID** é um identificador de usuário escopado ao portfólio de negócios, entregue no campo `user_id` dos webhooks. Ele é útil quando o usuário adotou um **username** e o telefone (`wa_id`) pode não vir no payload. Entenda o conceito em [BSUID e identificadores de usuário](/docs/positus/integracao/bsuid).
</Info>

<Warning>
**Precedência quando `to` e `recipient` coexistem:** se você enviar os dois campos na mesma requisição, o **telefone (`to`) prevalece** — a mensagem é entregue ao telefone. Por isso, envie **apenas um dos dois**: só `to` (telefone) ou só `recipient` (BSUID/parent BSUID).
</Warning>

<Note>
Números **on-premises** aceitam somente `to` (telefone). O envio por `recipient` (BSUID) está disponível apenas para números na **Cloud API** (Meta).
</Note>

#### Request Body (envio para BSUID)
...
#### Response (envio para BSUID)

A resposta é a mesma dos demais envios, mas o bloco `contacts` — repassado da Meta — traz o identificador que você usou em `input`, além do telefone (`wa_id`, quando disponível) e do BSUID (`user_id`).
...
<Note>
**Casos especiais que exigem telefone (não aceitam BSUID):** templates de **autenticação** dos tipos **one-tap**, **zero-tap** e **copy-code**. Nesses casos, informe sempre `to` (telefone). Se você enviar um BSUID onde ele não é suportado, a Meta responde com o erro `131062`.
</Note>
...
| Name       | Type   | Description                                          |
| ---------- | ------ | ---------------------------------------------------- |
| message_id | string | **Obrigatório.** ID da mensagem recebida do cliente. |
...
<Info>
O destinatário pode ser informado por telefone (`to`) **ou** por BSUID (`recipient`), como descrito em [Destinatário: telefone (`to`) ou BSUID (`recipient`)](#recipient). **Exceção:** templates de **autenticação** one-tap, zero-tap e copy-code exigem o **telefone** (`to`) e não aceitam BSUID.
</Info>
...
## Upload de Mídia <a href="#upload-de-midia" id="upload-de-midia"></a>

`POST` `https://api.positus.global/v2/whatsapp/numbers/{{chave}}/send-media`

Envia um arquivo para a Meta e devolve o **ID da mídia**. Use esse ID no campo `id` das mensagens de mídia (`image`, `document`, `video`, `audio`, `sticker`) quando você não quiser expor uma URL pública no campo `link`.
...
A requisição é **multipart/form-data**, com o arquivo no campo `media`.

| Name  | Type | Description                                                                                 |
| ----- | ---- | --------------------------------------------------------------------------------------------- |
| media | file | **Obrigatório.** Arquivo a ser enviado. Tamanho máximo de 16000 KB (aproximadamente 16 MB). |

```bash
curl -X POST "https://api.positus.global/v2/whatsapp/numbers/{{chave}}/send-media" \
  -H "Authorization: Bearer SEU_TOKEN" \
  -F "media=@/caminho/para/arquivo.pdf"
```

<Info>
A Positus valida apenas o tamanho do arquivo. Os **tipos aceitos** são definidos pela Meta de acordo com o tipo de mensagem que você pretende enviar (por exemplo, `image/jpeg`, `image/png`, `application/pdf`, `video/mp4`, `audio/ogg`). Um arquivo de tipo não suportado é recusado pela Meta e retorna **422**.
</Info>
...
<Tabs>
  <Tab title="200">
    O campo `id` é o ID da mídia na Meta, pronto para ser usado no envio de mensagens.
  ...
  <Tab title="422">
    A Meta recusou o upload. O objeto `error` é repassado exatamente como veio da Meta.
...
Exemplo de envio de mensagem usando o `id` retornado:
...
<Warning>
O ID da mídia é **temporário** na Meta. Faça o upload e envie a mensagem na sequência, sem armazenar o ID por longos períodos.
</Warning>

## Consultar Mídia por ID <a href="#consultar-midia-por-id" id="consultar-midia-por-id"></a>

`GET` `https://api.positus.global/v2/whatsapp/numbers/{{chave}}/media-by-id/{{messages.type.id}}`

Retorna os **metadados da mídia em JSON**, incluindo uma URL da Positus para baixar o arquivo.

<Info>
**Diferença para o [Download Midia](#download-midia):**

- `GET /media/{{id}}` devolve o **conteúdo binário** do arquivo, com o `Content-Type` correspondente.
- `GET /media-by-id/{{id}}` devolve um **JSON** com `id`, `url`, `mime_type`, `sha256`, `size` e `messaging_product`. Use esta rota quando você prefere receber um link para baixar o arquivo depois, em vez de receber os bytes na própria resposta.
</Info>
...
| ----- | ------ | ------------------------------------------------------------------------------------ |
| Chave | string | Código único por número de WhatsApp                                                  |
| id    | string | ID da mídia na Meta (recebido no webhook ou retornado pelo [Upload de Mídia](#upload-de-midia)) |
...
<Tabs>
  ...
  <Tab title="404">
    Mídia não encontrada ou já expirada na Meta. O corpo é o erro retornado pela Meta.
  ...
  <Tab title="500">
    A mídia foi localizada na Meta, mas o download do arquivo falhou. O corpo traz os metadados retornados pela Meta.
...
| ------------------- | -------------- | ---------------------------------------------------------------------------------------------- |
| `id`                | string         | ID da mídia na Meta.                                                                             |
| `url`               | string         | URL da Positus para baixar o arquivo.                                                            |
| `mime_type`         | string         | Tipo do arquivo (por exemplo, `image/jpeg`, `application/pdf`).                                  |
| `sha256`            | string \| null | Hash informado pela Meta. Vem `null` quando a mídia já estava armazenada na Positus.             |
| `size`              | integer        | Tamanho do arquivo em bytes.                                                                     |
| `messaging_product` | string         | Sempre `whatsapp`.                                                                               |

<Note>
Se a mídia já tiver sido baixada antes, a Positus responde direto do próprio armazenamento, sem consultar a Meta. Nesse caso o campo `sha256` vem `null`. Caso contrário, a Positus busca a mídia na Meta, armazena o arquivo e devolve os metadados completos.
</Note>

<Warning>
O ID da mídia na Meta é **efêmero** e cada consulta consome a cota de requisições do aplicativo. Baixe e guarde o arquivo na sua aplicação em vez de consultar o mesmo ID repetidamente. Quando a Meta responde **429**, respeite o cabeçalho `Retry-After` antes de tentar de novo.
</Warning>
