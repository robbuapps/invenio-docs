...
### Recipient: phone (`to`) or BSUID (`recipient`) <a href="#recipient" id="recipient"></a>

Every message-sending route accepts **two ways** to identify the recipient. Use **one OR the other**:

| Field       | Type   | Requirement                          | Description                                                                                                                                             |
| ----------- | ------ | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `to`        | string | Required if `recipient` is omitted   | Recipient's phone number in international format (e.g. `+5511999999999`).                                                                              |
| `recipient` | string | Optional — send instead of `to`          | Recipient's **BSUID** (*Business-Scoped User ID*) or **parent BSUID** (e.g. `BR.1234567890`). See [BSUID and user identifiers](/en/positus/integration/bsuid). |

<Info>
The **BSUID** is a user identifier scoped to the business portfolio, delivered in the `user_id` field of webhooks. It is useful when the user has adopted a **username** and the phone number (`wa_id`) may not be present in the payload. Learn the concept in [BSUID and user identifiers](/en/positus/integration/bsuid).
</Info>
...
<Info>
The recipient can be provided by phone (`to`) **or** by BSUID (`recipient`), as described in [Recipient: phone (`to`) or BSUID (`recipient`)](#recipient). **Exception:** **authentication** templates of the one-tap, zero-tap and copy-code types require the **phone** (`to`) and do not accept BSUID.
</Info>
...
Uploads a file to Meta and returns the **media ID**. Use this ID in the `id` field of media messages (`image`, `document`, `video`, `audio`, `sticker`) when you do not want to expose a public URL in the `link` field.
...
| Name  | Type | Description                                                                    |
| ----- | ---- | -------------------------------------------------------------------------------- |
| media | file | **Required.** File to upload. Maximum size of 16000 KB (approximately 16 MB).      |
...
<Tabs>
  ...
  <Tab title="422">
    Meta refused the upload. The `error` object is forwarded exactly as returned by Meta.
...
## Get Media by ID <a href="#get-media-by-id" id="get-media-by-id"></a>

`GET` `https://api.positus.global/v2/whatsapp/numbers/{{chave}}/media-by-id/{{messages.type.id}}`

Returns the **media metadata as JSON**, including a Positus URL to download the file.

<Info>
**Difference from [Download Media](#download-media):**
...
| ----- | ------ | ---------------------------------------------------------------------------------------- |
| Chave | string | Unique code per WhatsApp number                                                            |
| id    | string | Media ID at Meta (received in the webhook or returned by [Upload Media](#upload-media))    |
...
<Tabs>
  ...
  <Tab title="404">
    Media not found or already expired at Meta. The body is the error returned by Meta.
  ...
  <Tab title="500">
    The media was found at Meta, but downloading the file failed. The body carries the metadata returned by Meta.
...
| ------------------- | -------------- | ---------------------------------------------------------------------------------------- |
| `id`                | string         | Media ID at Meta.                                                                          |
| `url`               | string         | Positus URL to download the file.                                                          |
| `mime_type`         | string         | File type (for example, `image/jpeg`, `application/pdf`).                                  |
| `sha256`            | string \| null | Hash informed by Meta. Comes as `null` when the media was already stored at Positus.       |
